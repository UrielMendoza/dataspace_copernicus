...
# ------------------------------------------------------------------------------
def search_products_geom(geom, start_date, end_date, search_string=None, product_type="L1C", satellite=None, query_args=None) -> dict:
  '''
  Searches Copernicus DataSpace for Sentinel-2 datasets (products) for a given
  geometry and date. Alternatively, if search_string is given pass that directly
  to the Copernicus API (ignoring all other arguments).
  Parameters:
    geom : GeoJSON
  ...
  # Build the URL with the search parameters
  url = f"https://catalogue.dataspace.copernicus.eu/odata/v1/Products?$filter=OData.CSC.Intersects(area=geography'SRID=4326;{geom}') and ContentDate/Start gt {start_date}T00:00:00.000Z and ContentDate/Start lt {end_date}T00:00:00.000Z and contains(Name, '{level}')"
...
# ------------------------------------------------------------------------------
def download_products_geom(products, datadir, unzip=False, max_retries=5, verbose=True):
  '''
  Downloads a set of Sentinel-2 products.
  Parameters:
    products : dict
  ...
  if unzip == False:
    with open(f"{datadir}T{name}.zip", 'wb') as file:
        print(f'Downloading {origin_date} {id}...')
        for chunk in response.iter_content(chunk_size=8192):
            if chunk:
                file.write(chunk)
  # Wait to download the file
...
def search_and_download_datasets_geom(geom, start_date, end_date, datadir, unzip=False, max_retries=3, verbose=True, query_args=None):
  '''
    Searches Copernicus DataSpace for Sentinel-2 datasets (products) for a given geometry and date, and downloads them.
    Parameters:
      geom : GeoJSON
        The geometry to search for
      start_date : datetime object
  ...
  '''
  # Find products
  products = search_products_geom(geom, start_date, end_date, query_args=query_args)
  # Download products
  download_products_geom(products, datadir, unzip=unzip, max_retries=max_retries, verbose=verbose)
  # Empty the products dictionary
  products = None
...
if __name__ == "__main__":
  ...
  # Geometry
  point = 'POINT(-99.13%2019.41)'

  print('Sentinel-2\nStart:',start_date,'\nEnd:',end_date)

  # ------------------------------------------------------------------------------
  #search_and_download_datasets(tiles, start_date, end_date, datadir, unzip=False)
  search_and_download_datasets_geom(point, start_date, end_date, datadir, unzip=False)
